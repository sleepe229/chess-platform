...
public class NatsConfig {
    ...
    @Value("${nats.connection-name:analytics-service}")
    ...
    private void initializeStreams(Connection connection) {
        try {
            ...
            ensureStream(jsm, NatsSubjects.STREAM_ANALYTICS, new String[]{
                    NatsSubjects.ANALYTICS_REQUESTED,
                    NatsSubjects.ANALYTICS_COMPLETED,
                    NatsSubjects.ANALYTICS_FAILED
            });
        } catch (Exception e) {
            log.warn("Failed to initialize JetStream streams (analytics-service). NATS will still run without persistence.", e);
        }
    }
    ...
    private void ensureStream(JetStreamManagement jsm, String streamName, String[] subjects) {
        ...
        try {
            ...
            log.debug("JetStream stream {} already exists: {}", streamName, e.getMessage());
        }
    }
}
