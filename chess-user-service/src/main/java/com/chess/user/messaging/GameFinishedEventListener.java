package com.chess.user.messaging;

import com.chess.common.messaging.ProcessedEventStore;
import com.chess.events.common.EventEnvelope;
import com.chess.events.constants.NatsSubjects;
import com.chess.events.game.GameFinishedEvent;
import com.chess.user.service.RatingService;
import com.chess.user.service.UserService;
import com.fasterxml.jackson.databind.ObjectMapper;
import io.nats.client.Connection;
import io.nats.client.Dispatcher;
import io.nats.client.JetStream;
import io.nats.client.JetStreamSubscription;
import io.nats.client.Message;
import io.nats.client.PushSubscribeOptions;
import io.nats.client.api.AckPolicy;
import io.nats.client.api.ConsumerConfiguration;
import io.nats.client.api.DeliverPolicy;
import io.nats.client.api.ReplayPolicy;
import jakarta.annotation.PostConstruct;
import jakarta.annotation.PreDestroy;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.boot.autoconfigure.condition.ConditionalOnProperty;
import org.springframework.stereotype.Component;

import java.nio.charset.StandardCharsets;
import java.time.Duration;
import java.util.UUID;

@Slf4j
@Component
@RequiredArgsConstructor
@ConditionalOnProperty(name = "nats.enabled", havingValue = "true", matchIfMissing = true)
public class GameFinishedEventListener {

    private static final int MAX_DELIVER = 5;
    private static final String CONSUMER = "user-service-game-finished";

    private final Connection natsConnection;
    private final JetStream jetStream;
    private final ObjectMapper objectMapper;
    private final RatingService ratingService;
    private final UserService userService;
    private final ProcessedEventStore processedEventStore;

    private Dispatcher dispatcher;
    private JetStreamSubscription subscription;

    @PostConstruct
    public void init() {
        try {
            ConsumerConfiguration config = ConsumerConfiguration.builder()
                    .durable(CONSUMER)
                    .ackPolicy(AckPolicy.Explicit)
                    .ackWait(Duration.ofSeconds(30))
                    .maxDeliver(MAX_DELIVER)
                    .backoff(
                            Duration.ofSeconds(1),
                            Duration.ofSeconds(5),
                            Duration.ofSeconds(15),
                            Duration.ofSeconds(30),
                            Duration.ofSeconds(60)
                    )
                    .deliverPolicy(DeliverPolicy.All)
                    .replayPolicy(ReplayPolicy.Instant)
                    .build();
            PushSubscribeOptions opts = PushSubscribeOptions.builder().configuration(config).build();

            dispatcher = natsConnection.createDispatcher();
            subscription = jetStream.subscribe(NatsSubjects.GAME_FINISHED, dispatcher, this::handleMessage, false, opts);

            log.info("JetStream subscribed to {}", NatsSubjects.GAME_FINISHED);
        } catch (Exception e) {
            log.error("Failed to initialize JetStream subscription to {}", NatsSubjects.GAME_FINISHED, e);
        }
    }

    @PreDestroy
    public void cleanup() {
        if (subscription != null) {
            try {
                subscription.unsubscribe();
            } catch (Exception ignored) {
            }
        }
        if (dispatcher != null) {
            try {
                dispatcher.unsubscribe(NatsSubjects.GAME_FINISHED);
            } catch (Exception ignored) {
            }
        }
    }

    private void handleMessage(Message message) {
        String json = new String(message.getData(), StandardCharsets.UTF_8);
        EventEnvelope<GameFinishedEvent> envelope;
        try {
            envelope = objectMapper.readValue(json,
                    objectMapper.getTypeFactory().constructParametricType(EventEnvelope.class, GameFinishedEvent.class));
        } catch (Exception e) {
            log.error("Failed to deserialize GameFinished envelope, skipping", e);
            safeAck(message);
            return;
        }

        GameFinishedEvent event = envelope.getPayload();
        if (event == null) {
            safeAck(message);
            return;
        }

        if (envelope.getEventId() != null && processedEventStore.isProcessed(CONSUMER, envelope.getEventId())) {
            safeAck(message);
            return;
        }

        try {
            UUID gameId = UUID.fromString(event.getGameId());
            UUID whiteId = UUID.fromString(event.getWhitePlayerId());
            UUID blackId = UUID.fromString(event.getBlackPlayerId());

            // Stats are updated for all games; rating only for rated games.
            // Map to per-player result strings (WIN/LOSS/DRAW)
            String whiteResult = mapResultForWhite(event.getResult());
            String blackResult = switch (whiteResult) {
                case "WIN" -> "LOSS";
                case "LOSS" -> "WIN";
                default -> "DRAW";
            };
            userService.updateGameStats(whiteId, whiteResult);
            userService.updateGameStats(blackId, blackResult);

            if (Boolean.TRUE.equals(event.getRated())) {
                double whiteScore = scoreForWhite(event.getResult());
                double blackScore = 1.0 - whiteScore;
                String timeControl = event.getTimeControlType();

                ratingService.updateRatingAfterGame(whiteId, blackId, timeControl, whiteScore, gameId);
                ratingService.updateRatingAfterGame(blackId, whiteId, timeControl, blackScore, gameId);
            }

            if (envelope.getEventId() != null) {
                processedEventStore.markProcessed(CONSUMER, envelope.getEventId());
            }
            message.ack();
        } catch (Exception e) {
            int delivered = 1;
            if (message.metaData() != null) {
                try {
                    delivered = Math.toIntExact(message.metaData().deliveredCount());
                } catch (ArithmeticException ignored) {
                    delivered = Integer.MAX_VALUE;
                }
            }
            log.warn("Failed to process GameFinished (deliveredCount={}): {}", delivered, e.getMessage(), e);

            if (delivered >= MAX_DELIVER) {
                try {
                    String dlqSubject = "domain.dlq." + (envelope.getEventType() != null ? envelope.getEventType() : "GameFinished");
                    jetStream.publish(dlqSubject, message.getData());
                    log.error("Sent to DLQ subject={} eventId={}", dlqSubject, envelope.getEventId());
                } catch (Exception dlqEx) {
                    log.error("Failed to publish GameFinished to DLQ, will ACK to stop poison loop", dlqEx);
                } finally {
                    safeAck(message);
                }
            } else {
                try {
                    message.nak();
                } catch (Exception nakEx) {
                    log.warn("Failed to NAK message, ACKing to avoid stuck redelivery", nakEx);
                    safeAck(message);
                }
            }
        }
    }

    private double scoreForWhite(String result) {
        if ("1-0".equals(result)) return 1.0;
        if ("0-1".equals(result)) return 0.0;
        return 0.5;
    }

    private String mapResultForWhite(String result) {
        if ("1-0".equals(result)) return "WIN";
        if ("0-1".equals(result)) return "LOSS";
        return "DRAW";
    }

    private void safeAck(Message message) {
        try {
            message.ack();
        } catch (Exception ignored) {
        }
    }
}

